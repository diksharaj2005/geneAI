      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className={`h-2 w-full rounded-full ${getDemandLevelColor(insights.demandLevel)}`} />
          </CardContent>
        </Card>
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={salaryData}>
                ...
                />
                <Bar dataKey="min" fill="#686ddd" name="Min Salary (K)" />
                <Bar dataKey="median" fill="#6475ac" name="Median Salary (K)" />
                <Bar dataKey="max" fill="#945fb6ff" name="Max Salary (K)" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
            ...
            <ul className='space-y-4'>
              {insights.keyTrends.map((trend, index) => (
                <li key={index} className="flex items-start space-x-2">
                  <div className='h-2 w-2 mt-2 rounded-full bg-purple-700'/>
                  <span>{trend}</span>
                </li>
))}
            </ul>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {insights.recommendedSkills.map((skill) => (
                <Badge key={skill} variant="outline" className='bg-black border-white'>
                  {skill}
                </Badge>
              ))}
            </div>
